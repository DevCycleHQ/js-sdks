...
if [[ -z "$VERSION_INCREMENT" ]]; then
  echo "Defaulting to patch version increment"
...
for PROJECT in "${AFFECTED_PROJECTS[@]}"; do
  ...
  PACKAGE=$(jq -r '.name' "$FILEPATH/../package.json")
...
yarn lerna version "$VERSION_INCREMENT" --force-publish="$CSV_PACKAGES" --message "chore(release): publish" "$@" --no-push
