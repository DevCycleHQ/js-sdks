...
import DevCycleAngularProvider from '../src/index'
import {
    Client,
    OpenFeature,
...
async function initOFClient(): Promise<{
    ofClient: Client
    provider: DevCycleAngularProvider
}> {
    const options = { logger }
    OpenFeature.setContext({ targetingKey: 'node_sdk_test' })
    const provider = new DevCycleAngularProvider('dvc_client_sdk_key', options)
...
describe('DevCycleReactProvider Unit Tests', () => {
    ...
    describe('Tracking Events', () => {
        let trackMock: any
        let openFeatureClient: Client
        let provider: DevCycleAngularProvider
