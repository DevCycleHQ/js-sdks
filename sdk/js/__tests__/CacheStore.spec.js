...
describe('CacheStore tests', () => {
    ...
    it('should save user to local storage using user-specific key', async () => {
        const store = new CacheStore(localStorage, mockLogger)
        const user = new DVCPopulatedUser({ user_id: 'user1' }, {})
        store.saveUser(user)
        const expectedKey = `${StoreKey.User}.user1`
        expect(localStorage.save).toHaveBeenCalledWith(expectedKey, user)
    })

    it('should load user from local storage using user-specific key', async () => {
        // Mock no legacy user data so migration doesn't interfere
        localStorage.load.mockImplementation((key) => {
            if (key === StoreKey.User) return null // No legacy user
            if (key === StoreKey.IdentifiedConfig || key === StoreKey.AnonymousConfig) return null // No legacy configs
            return null
        })
        
        const store = new CacheStore(localStorage, mockLogger)
        await store.loadUser('user1')
        const expectedKey = `${StoreKey.User}.user1`
        expect(localStorage.load).toHaveBeenCalledWith(expectedKey)
    })

    it('should migrate legacy user data when loading user', async () => {
        const legacyUser = new DVCPopulatedUser({ user_id: 'legacy_user' }, {})
        
        localStorage.load.mockImplementation((key) => {
            if (key === StoreKey.User) return legacyUser // Legacy user exists
            if (key === `${StoreKey.User}.legacy_user`) return null // No new format yet
            // No legacy configs
            if (key === StoreKey.IdentifiedConfig || key === StoreKey.AnonymousConfig) return null
            return null
        })
        
        const store = new CacheStore(localStorage, mockLogger)
        
        // Load user should trigger migration
        await store.loadUser('legacy_user')
        
        // Verify migration attempted to load legacy format
        expect(localStorage.load).toHaveBeenCalledWith(StoreKey.User)
        
        // And migrated to new format
        const newKey = `${StoreKey.User}.legacy_user`
        expect(localStorage.save).toHaveBeenCalledWith(newKey, legacyUser)
