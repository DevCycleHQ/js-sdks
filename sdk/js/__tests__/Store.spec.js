...
describe('Store tests', () => {
    ...
    it('should save config to local storage', async () => {
        ...
        expect(localStorage.save).toBeCalledWith(StoreKey.IdentifiedConfig, config)
        expect(localStorage.save).toBeCalledWith(`${StoreKey.IdentifiedConfig}.fetch_date`, Date.now())
        expect(localStorage.save).toBeCalledWith(`${StoreKey.IdentifiedConfig}.user_id`, user.user_id)
    })
    ...
    it('should load config from local storage if user id and config date are validated', async () => {
        ...
        expect(localStorage.load).toBeCalledWith(`${StoreKey.IdentifiedConfig}.user_id`)
        expect(localStorage.load).toBeCalledWith(`${StoreKey.IdentifiedConfig}.fetch_date`)
