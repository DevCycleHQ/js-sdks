...
describe('ConfigRequestConsolidator Tests', () => {
  ...
  beforeEach(() => {
    jest.resetAllMocks()
    requestConsolidator = new ConfigRequestConsolidator(requestFn, receiveFn, {
      user_id: 'initial',
    })
  })
  ...
  it('queues extra operations with extraParams set', async () => {
    requestFn.mockResolvedValueOnce('test1')
    requestFn.mockResolvedValueOnce('test2')
    const promise1 = requestConsolidator.queue(
      { user_id: 'user1' },
      { sse: true, lastModified: 2345, etag: 'etag' },
    )
    const promise2 = requestConsolidator.queue({ user_id: 'user2' })
    const promise3 = requestConsolidator.queue(
      { user_id: 'user3' },
      { sse: true, lastModified: 5678, etag: 'etag' },
    )
