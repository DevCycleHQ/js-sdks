...
describe('EventQueue tests', () => {
    ...
    describe('queueAggregateEvents', () => {
        it('should throw if event does not have both target and type defined', () => {
            const eventNoTarget = { type: EventTypes.variableEvaluated, clientDate: Date.now() }
            const eventNoType = { target: 'variable_key', clientDate: Date.now() }
            expect(() => eventQueue.queueAggregateEvent(eventNoTarget)).toThrow(expect.any(Error))
            expect(() => eventQueue.queueAggregateEvent(eventNoType)).toThrow(expect.any(Error))
        })

        it('aggregates evaluated events and default events', async () => {
            const event1 = { type: EventTypes.variableEvaluated, target: 'dummy_key1', clientDate: Date.now() }
            const event2 = { ...event1, clientDate: Date.now() }
            const defaultEvent1 = { type: EventTypes.variableDefaulted, target: 'dummy_key1', clientDate: Date.now() }
            const defaultEvent2 = { type: EventTypes.variableDefaulted, target: 'dummy_key1', clientDate: Date.now() }
