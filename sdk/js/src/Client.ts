...
> implements Client<Variables>
{
  ...
  private handleConfigReceived(
    ...
    if (this.user != user || !this.userSaved) {
      ...
      if (
        ...
        saveEntity(this.user, this.sdkKey, this.logger, this.options).then(
          (res) => this.logger.info(`Saved response entity! ${res}`),
        )
      }
  ...
  private async initializeClient() {
    return this.requestConsolidator
      .queue(this.user)
      .then(() => {
