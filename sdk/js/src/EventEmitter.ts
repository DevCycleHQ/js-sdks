...
export class EventEmitter {
  ...
  emitVariableUpdates(
    ...
    keys.forEach((key) => {
      ...
      if (
        JSON.stringify(oldVariableValue) !== JSON.stringify(newVariableValue)
      ) {
        const variables =
          variableDefaultMap[key] && Object.values(variableDefaultMap[key])
        if (variables) {
          newVariables = true
          variables.forEach((variable) => {
            variable.value = newVariableValue ?? variable.defaultValue
            variable.isDefaulted =
              newVariableValue === undefined || newVariableValue === null
