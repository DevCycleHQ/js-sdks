...
describe('EventQueue Unit Tests', () => {
  ...
  it('should setup Event Queue and not process automatic events if disableAutomaticEventLogging is true', async () => {
    publishEvents_mock.mockResolvedValue(mockFetchResponse({ status: 201 }))
  ...
  describe('Max queue size tests', () => {
    it('should not queue user event if user event and aggregate event queue exceeds max event queue size', () => {
      ...
      expect(logger.warn).toBeCalledWith(
        expect.stringContaining('Max event queue size reached, dropping event'),
      )
      expect(getBucketingLib().eventQueueSize(sdkKey)).toEqual(2000)
    })
  ...
  describe('EventQueue init validation', () => {
    it('should validate flushEventsMS', () => {
      ...
      expect(
        ...
        `eventFlushIntervalMS: ${10 * 60 * 1000} must be smaller than 1 minute`,
      )
    })
