...
module.exports = composePlugins(withNx(), (config, passedArgs) => {
    ...
    const workspaceRoot = context ? (context.workspaceRoot || context.root) : undefined;
    ...
    if (options && options.index && workspaceRoot) {
        ...
        console.log(`[DEBUG] HtmlWebpackPlugin: Using template path: ${templatePath}`)
        config.plugins.push(
        ...
        if (!workspaceRoot) {
            console.error('[Error] workspaceRoot could not be determined from context.')
            if (context) {
                console.error(`[DEBUG] context.workspaceRoot: ${context.workspaceRoot}, context.root: ${context.root}`)
            } else {
