...
async function startDVC() {
    const dvcClient = await initialize(DVC_SERVER_SDK_KEY, { logLevel: 'info' }).onClientInitialized()
    ...
    const partyTimeFlag = await openFeatureClient.getBooleanValue('boolean-flag', false)
    if (partyTimeFlag) {
    ...
    const defaultVariable = await openFeatureClient.getBooleanValue('not-real', true)
