...
describe('murmurhash V3 Assembly Script implementation', () => {
    it('should return the correct hash for simple strings with a varying seed', () => {
        for (let i = 0; i < 2000; i++) {
            expect(murmurhashV3_js('some-long-ascii-string', i)).toContain(
                `${murmurhash.v3('some-long-ascii-string', i)}`,
            )
            expect(murmurhashV3_js('some-long-ascii-string???', i)).toContain(
                `${murmurhash.v3('some-long-ascii-string???', i)}`,
            )
            ...
            expect(
                murmurhashV3_js('some-long-ascii-string**********', i),
            ).toContain(
                `${murmurhash.v3('some-long-ascii-string**********', i)}`,
            )
        }
    ...
    it('should return the correct hash for a random string with a varying seed', () => {
        for (let i = 0; i < 20000; i++) {
            ...
            expect(murmurhashV3_js(testString, i)).toContain(
                `${murmurhash.v3(testString, i)}`,
            )
        }
    })
    ...
    it('should return the correct result for a string that exceeds the maximum buffer size', () => {
        ...
        expect(murmurhashV3_js(testString, 0)).toContain(
            `${murmurhash.v3(testString, 0)}`,
        )
    })
