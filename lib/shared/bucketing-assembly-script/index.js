...
const instantiate = async (debug = false, imports = {}) => {
    ...
    let compiled
    if (isEdgeRuntime()) {
        // In Edge Runtime, we need to use the WASM binary directly
        const wasmBinary = require(`./build/bucketing-lib.${releaseStr}.wasm`)
        compiled = await globalThis.WebAssembly.compile(wasmBinary)
    } else {
        /**
