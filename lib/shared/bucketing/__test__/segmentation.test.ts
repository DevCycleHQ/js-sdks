...
describe('SegmentationManager Unit Test', () => {
    ...
    describe('evaluateOperator', () => {
        ...
        describe('evaluateOperator should handle audienceMatch filter', () => {
            const filters = [
                { type: FilterType.user, subType: UserSubType.country, comparator: FilterComparator['='], values: ['Canada'] },
                { type: FilterType.user, subType: UserSubType.email, comparator: FilterComparator['='], values: ['dexter@smells.nice', 'brooks@big.lunch'] },
                { type: FilterType.user, subType: UserSubType.appVersion, comparator: FilterComparator['>'], values: ['1.0.0'] }
            ]
            const operator = {
                filters,
                operator: AudienceOperator.and
            ...
            it('should not pass seg when not in audience for happy path case', () => {
                ...
                assert.strictEqual(false, segmentation.evaluateOperator(
                    { data, operator: audienceMatchOperatorNotEqual, audiences, featureId, isOptInEnabled }))
            })
            it('should pass seg for nested audiences', () => {
                ...
                assert.strictEqual(true, segmentation.evaluateOperator(
                    { data, operator: nestedAudienceMatchOperator, audiences, featureId, isOptInEnabled }))
            })
            ...
            it('should pass seg for an AND operator with multiple values', () => {
                ...
                const filters = [
                    { type: FilterType.user, subType: UserSubType.country, comparator: FilterComparator['='], values: ['USA'] },
                ]
                ...
                assert.strictEqual(true, segmentation.evaluateOperator(
                    { data, operator: audienceMatchOperatorMultiple, audiences, featureId, isOptInEnabled }))
            })
    ...
    describe('checkStringsFilter', () => {
        ...
        it('should return true if exists filter and value', () => {
            const filter = { type: 'user', comparator: 'exist' } as AudienceFilterOrOperator
            assert.strictEqual(true, segmentation.checkStringsFilter('string', filter))
        })
        ...
        it('should return false if not exists filter and value', () => {
            const filter = { type: 'user', comparator: '!exist' } as AudienceFilterOrOperator
            assert.strictEqual(false, segmentation.checkStringsFilter('string', filter))
        })
        ...
        it('should return true if browser filter works', () => {
            ...
            assert.strictEqual(true, segmentation.checkStringsFilter('Chrome', filter))
        })
        it('should return true if browser device type filter works', () => {
            ...
            assert.strictEqual(true, segmentation.checkStringsFilter('Desktop', filter))
        })
        it('should return true if contains filter and value contains', () => {
            const filter = { type: 'user', comparator: 'contain', values: ['hello'] } as AudienceFilterOrOperator
            assert.strictEqual(true, segmentation.checkStringsFilter('helloWorld', filter))
        })
        ...
        it('should return false if not contains filter and not value', () => {
            const filter = { type: 'user', comparator: '!contain', values: ['hello'] } as AudienceFilterOrOperator
            assert.strictEqual(false, segmentation.checkStringsFilter('hello', filter))
        })
        ...
        it('should return false if string is not equal', () => {
            const filter = { type: 'user', comparator: '=', values: ['Male'] } as AudienceFilterOrOperator
            assert.strictEqual(false, segmentation.checkStringsFilter('Female', filter))
        })
    ...
    describe('checkNumberFilter', () => {
        it('should return false if operator is not valid', () => {
            ...
            assert.strictEqual(false, segmentation.checkNumberFilter(0, [0], 0 as unknown as string))
        })
        ...
        it('should return false if values are not equal', () => {
            ...
            assert.strictEqual(false, segmentation.checkNumberFilter(10, [-10, -12], '='))
        })
        it('should return true if values are not equal', () => {
            assert.strictEqual(true, segmentation.checkNumberFilter(0, [10], '!='))
            assert.strictEqual(true, segmentation.checkNumberFilter(10, [-10, -12], '!='))
        })
        it('should return true if values are not equal', () => {
    ...
    describe('checkVersionFilters', () => {
        it('should return false if filter and version is null', () => {
            ...
            assert.strictEqual(false, segmentation.checkVersionFilters(null as unknown as string, filter))
            assert.strictEqual(false, segmentation.checkVersionFilters(null as unknown as string, filter1))
        })
        it('should return true if filter equals version', () => {
            const filter = { type: 'user', comparator: '=', values: ['1.1.2', '1.1.3'] } as AudienceFilterOrOperator
            assert.strictEqual(true, segmentation.checkVersionFilters('1.1.2', filter))
        })
        ...
        it('should return true if filter equals version non semver', () => {
            ...
            assert.strictEqual(true, segmentation.checkVersionFilters('1.1.2.12', filter))
            assert.strictEqual(true, segmentation.checkVersionFilters('31.331.2222.12', filter2))
            assert.strictEqual(true, segmentation.checkVersionFilters('1.1.2.12.1.2.3', filter3))
        })
        it('should return false if filter does not equals version', () => {
            ...
            assert.strictEqual(false, segmentation.checkVersionFilters('1.1.2', filter))
        })
    })
    ...
    describe('checkVersionFilter', () => {
        it('should return true if string versions equal', () => {
            ...
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1', ['1.1'], '='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.1', ['1.1.1'], '='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.', ['1.1'], '='))
        })
        it('should return false if string versions not equal', () => {
            assert.strictEqual(false, segmentation.checkVersionFilter(null as unknown as string, ['2'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1', ['2'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.2'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.1.1'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.', ['1.1.1'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.1', ['1.1'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.1', ['1.1.'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.1', ['1.2.3'], '='))
        })
        it('should return false if string versions not equal', () => {
            assert.strictEqual(false, segmentation.checkVersionFilter('1', ['1'], '!='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.1'], '!='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.1', ['1.1.1'], '!='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.', ['1.1'], '!='))
        })
        it('should return true if string versions not equal', () => {
            assert.strictEqual(true, segmentation.checkVersionFilter('1', ['2'], '!='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1', ['1.2'], '!='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1', ['1.1.1'], '!='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.', ['1.1.1'], '!='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.1', ['1.1'], '!='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.1', ['1.1.'], '!='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.1', ['1.2.3'], '!='))
        })
        it('should return true if string versions greater than', () => {
            assert.strictEqual(false, segmentation.checkVersionFilter(null as unknown as string, ['1'], '>'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1', ['1'], '>'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.1'], '>'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.1', ['1.1.1'], '>'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.', ['1.1'], '>'))
        ...
        it('should return true if string versions greater than', () => {
            ...
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2', ['1.1'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('2.1', ['1.1'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.1', ['1.2'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.', ['1.1'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.1', ['1.1.1'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.2', ['1.2'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.2', ['1.2.1'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241', ['4.8'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241.2', ['4'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241.2', ['4.8'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241.2', ['4.8.2'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241.2', ['4.8.241.0'], '>'))
        })
        it('should return true if string versions greater than or equal', () => {
            ...
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.', ['1.1.1'], '>='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.1', ['1.2.3'], '>='))
            assert.strictEqual(false, segmentation.checkVersionFilter('4.8.241', ['4.9'], '>='))
            assert.strictEqual(false, segmentation.checkVersionFilter('4.8.241.2', ['5'], '>='))
            assert.strictEqual(false, segmentation.checkVersionFilter('4.8.241.2', ['4.9'], '>='))
            assert.strictEqual(false, segmentation.checkVersionFilter('4.8.241.2', ['4.8.242'], '>='))
            assert.strictEqual(false, segmentation.checkVersionFilter('4.8.241.2', ['4.8.241.5'], '>='))
        })
        it('should return true if string versions greater than or equal', () => {
            assert.strictEqual(true, segmentation.checkVersionFilter('1', ['1'], '>='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1', ['1.1'], '>='))
            ...
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241.2', ['4.8.2'], '>='))
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241.2', ['4.8.241.0'], '>='))
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241.2', ['4.8.241.2'], '>='))
        })
        it('should work if version has other characters', () => {
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.2', ['v1.2.1-2v3asda'], '>='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.2', ['v1.2.1-va1sda'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.1', ['v1.2.1-vasd32a'], '>='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.2.1', ['v1.2.1-vasda'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('v1.2.1-va21sda', ['v1.2.1-va13sda'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.2.0', ['v1.2.1-vas1da'], '>='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.1', ['v1.2.1- va34sda'], '<='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.0', ['v1.2.1-vas3da'], '<='))
        })
        it('should return false if string versions less than', () => {
            ...
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.', ['1.1.1'], '<'))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.1', ['1.2.3'], '<'))
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241.2', ['5'], '<'))
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241.2', ['4.9'], '<'))
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241.2', ['4.8.242'], '<'))
            assert.strictEqual(true, segmentation.checkVersionFilter('4.8.241.2', ['4.8.241.5'], '<'))
        })
        it('should return false if string versions less than', () => {
            assert.strictEqual(false, segmentation.checkVersionFilter(null as unknown as string, ['1'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1', ['1'], '<'))
            ...
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.1', ['1.1.1'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.', ['1.1'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('2', ['1'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.2', ['1.1'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('2.1', ['1.1'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.2.1', ['1.2'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.2.', ['1.1'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.2.1', ['1.1.1'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.2.2', ['1.2'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.2.2', ['1.2.'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.2.2', ['1.2.1'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('4.8.241.2', ['4'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('4.8.241.2', ['4.8'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('4.8.241.2', ['4.8.241'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('4.8.241.2', ['4.8.241.0'], '<'))
        })
        ...
        it('should return false if all numbers not equal array', () => {
            assert.strictEqual(false, segmentation.checkVersionFilter('1', ['2', '1.1'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.2', '1'], '='))
        })
        it('should return true if any string versions equal array', () => {
            assert.strictEqual(true, segmentation.checkVersionFilter('1', ['1', '1.1'], '='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1', ['1.1', '1'], '='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.1', ['1.1.1', '1.1'], '='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.', ['1.1', '1.1'], '='))
        })
        it('should return false if all string versions not equal array', () => {
            assert.strictEqual(false, segmentation.checkVersionFilter(null as unknown as string, ['2', '3'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1', ['2', '3'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.2', '1.2'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.1.1', '1.2'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.', ['1.1.1', '1.2'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.1', ['1.1', '1.1'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.1', ['1', '1.1.'], '='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.1', ['1.2.3', '1.'], '='))
        })
        it('should return false if multiple versions do not equal the version', () => {
            assert.strictEqual(false, segmentation.checkVersionFilter('1', ['2', '1'], '!='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.2', '1.1'], '!='))
        })
        it('should return true if multiple versions do not equal version', () => {
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1', ['1.1.1', '1.2'], '!='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.', ['1.1.1', '1'], '!='))
        })
        it('should return false if any string versions not greater than array', () => {
            assert.strictEqual(false, segmentation.checkVersionFilter('1', ['1', '1'], '>'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.1', '1.1.', '1.1'], '>'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1', ['2'], '>'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.1.0'], '>'))
        })
        it('should return true any if string versions greater than array', () => {
            assert.strictEqual(true, segmentation.checkVersionFilter('2', ['1', '2.0'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.1', ['1.2', '1.2'], '>'))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.2.', ['1.1', '1.9.'], '>'))
        })
        it('should return false if all string versions not greater than or equal array', () => {
            assert.strictEqual(false, segmentation.checkVersionFilter('1', ['2', '1.2'], '>='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.2'], '>='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1', ['1.1.1', '1.2'], '>='))
        })
        it('should return true if any string versions greater than or equal array', () => {
            assert.strictEqual(true, segmentation.checkVersionFilter('1', ['1', '1.1'], '>='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1', ['1.1', '1'], '>='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.1', ['1.2', '1.1.1'], '>='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.', ['1.1'], '>='))
            assert.strictEqual(true, segmentation.checkVersionFilter('2', ['1', '3'], '>='))
        })
        it('should return true if any string versions less than array', () => {
            assert.strictEqual(true, segmentation.checkVersionFilter('1', ['2', '1'], '<'))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1', ['1.2', '1.5'], '<'))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.', ['1.1.1'], '<'))
        })
        it('should return false if all string versions less than array', () => {
            assert.strictEqual(false, segmentation.checkVersionFilter('1', ['1', '1.0'], '<'))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.1.', ['1.1', '1.1.0'], '<'))
        })
        it('should return true if any string versions less than or equal array', () => {
            assert.strictEqual(true, segmentation.checkVersionFilter('1', ['1', '5'], '<='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1', ['1.1', '1.1.'], '<='))
            assert.strictEqual(true, segmentation.checkVersionFilter('1.1.', ['1.1.1', '1.1.'], '<='))
        })
        it('should return false if all string versions not less than or equal array', () => {
            assert.strictEqual(false, segmentation.checkVersionFilter('2', ['1', '1.9'], '<='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.2.1', ['1.2', '1.2'], '<='))
            assert.strictEqual(false, segmentation.checkVersionFilter('1.2.', ['1.1', '1.1.9'], '<='))
        })
    })
    ...
    describe('checkCustomData', () => {
        ...
        it('should return false if string value is not equal', () => {
            assert.strictEqual(false, segmentation.checkCustomData({ strKey: 'not value' }, filterStr))
        })
        ...
        it('should return true if bool value is equal', () => {
            assert.strictEqual(true, segmentation.checkCustomData({ boolKey: false }, filterBool))
        })
        it('should return false if bool value is not equal', () => {
            assert.strictEqual(false, segmentation.checkCustomData({ boolKey: true }, filterBool))
        })
        ...
        it('should return true if custom data contains value', () => {
            assert.strictEqual(true, segmentation.checkCustomData({ last_order_no: 'FP2423423' }, containsFilter))
        })
        ...
        it('should return true if custom data value exists', () => {
            assert.strictEqual(true, segmentation.checkCustomData({ field: 'something' }, existsFilter))
        })

        it('should return false if custom data value does not exist', () => {
            assert.strictEqual(false, segmentation.checkCustomData({ not_field: 'something' }, existsFilter))
        })
    })
    ...
    describe('parseUserAgent', () => {
        describe('Desktop User Agents', () => {
            ...
            it('should recognize a Chrome Desktop user agent', () => {
                ...
                expect(segmentation.parseUserAgent(chromeDesktopUA)).toEqual(outputChrome)
            })
            ...
            it('should recognize a Firefox Desktop user agent', () => {
                ...
                expect(segmentation.parseUserAgent(firefoxDesktopUA)).toEqual(outputFirefox)
            })
            ...
            it('should recognize a Safari Desktop user agent', () => {
                ...
                expect(segmentation.parseUserAgent(safariDesktopUA)).toEqual(outputSafari)
            })
            ...
            it('should recognize a Chromium Desktop user agent', () => {
                ...
                expect(segmentation.parseUserAgent(chromiumDesktopUA)).toEqual(outputChrome)
            })
            ...
            it('should recognize a Headless Chrome Desktop user agent', () => {
                ...
                expect(segmentation.parseUserAgent(chromeHeadlessDesktopUA)).toEqual(outputChrome)
            })
        ...
        describe('Mobile User Agents (iOS)', () => {
            ...
            it('should recognize a Chrome Mobile user agent', () => {
                ...
                expect(segmentation.parseUserAgent(chromeMobileUA)).toEqual(outputChrome)
            })
            ...
            it('should recognize a Firefox Mobile user agent', () => {
                ...
                expect(segmentation.parseUserAgent(firefoxMobileUA)).toEqual(outputFirefox)
            })
            ...
            it('should recognize a Safari Mobile user agent', () => {
                ...
                expect(segmentation.parseUserAgent(safariMobileUA)).toEqual(outputSafari)
            })
        })
        ...
        describe('Tablet User Agents (iOS)', () => {
            ...
            it('should recognize a Chrome Tablet user agent', () => {
                ...
                expect(segmentation.parseUserAgent(chromeTabletUA)).toEqual(outputChrome)
            })
            ...
            it('should recognize a Firefox Tablet user agent', () => {
                ...
                expect(segmentation.parseUserAgent(firefoxTabletUA)).toEqual(outputFirefox)
            })
            ...
            it('should recognize a Safari Tablet user agent', () => {
                ...
                expect(segmentation.parseUserAgent(safariTabletUA)).toEqual(outputSafari)
            })
        })
